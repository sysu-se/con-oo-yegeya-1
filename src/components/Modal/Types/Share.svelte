<script>
    ...
    import { getShareableCode, getShareableLink } from '@sudoku/stores/grid';
    import Clipboard from '../../Utils/Clipboard.svelte';

    export let data = {};
    export let hideModal;

    let code = getShareableCode();
    let link = getShareableLink();

    $: code = getShareableCode();
    ...
    $: mailToLink = 'mailto:?subject=A%20Sudoku%20puzzle%20for%20you&body=' + encodeURIComponent(`Here's a link to the current Sudoku puzzle:\n\n${link}`);
...
<div class="code-container">
    <input class="input code-field" type="text" readonly value={code} on:click={e => select(e.target)}>

    <button class="btn btn-copy" on:click={() => copyText(code)}>
        <svg class="icon-outline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
